...
info "This script will install joemob/nvimdots to:"
echo "${DEST_DIR}"
...
if [[ "${USE_SSH}" -eq "1" ]]; then
	clone_repo "git@github.com:joemob/nvimdots.git"
else
	clone_repo "https://github.com/joemob/nvimdots.git"
fi
...
- Project Homepage:
    ${tty_underline}https://github.com/joemob/nvimdots${tty_reset}
- Further documentation (including executables you ${tty_bold}must${tty_reset} install for full functionality):
    ${tty_underline}https://github.com/joemob/nvimdots/wiki/Prerequisites${tty_reset}
EOS
